<script lang="ts">
  ...
  async function handleSubmit(e: SubmitEvent) {
    ...
    loading = true;
    try {
      await apiFetch(`/families/${familySlug}/persons`, {
        method: "POST",
        body: JSON.stringify(form),
      });

      toast.success(`${form.full_name} berhasil ditambahkan ke silsilah`);
      open = false;
      // Reset form
      form = {
        full_name: "",
      ...
      toast.error(err.message || "Gagal menambahkan data person");
    } finally {
      loading = false;
    }
  }
...
<Dialog.Root bind:open>
  <Dialog.Trigger>
    {#snippet child({ props })}
      <Button
        {...props}
        class="gap-2 rounded-2xl h-11 px-6 font-bold shadow-lg shadow-primary/20 bg-primary hover:bg-primary/90 transition-all hover:scale-[1.02] cursor-pointer"
      >
        <UserPlus size={18} />
          ...
          <div class="flex items-start gap-4">
            ...
            <div class="min-w-0">
              <Dialog.Title class="text-2xl font-black tracking-tight"
                >Tambah Data Person</Dialog.Title
              >
              <Dialog.Description class="text-sm font-medium text-zinc-500">
                Lengkapi informasi dasar untuk anggota silsilah baru ini.
              </Dialog.Description>
            </div>
          </div>
        </Dialog.Header>
        ...
        <div class="grid gap-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div class="space-y-2">
                ...
                <Input
                  ...
                  placeholder="Contoh: Mbah Ngalidjo"
          ...
          <div class="space-y-3">
            ...
            <Textarea
              id="bio"
              bind:value={form.bio}
              placeholder="Ceritakan sejarah singkat atau kenangan tentang anggota keluarga ini..."
              class="min-h-[140px] rounded-[1.5rem] border-zinc-200 dark:border-zinc-800 p-5 focus-visible:ring-primary/20 bg-white dark:bg-zinc-950 shadow-sm text-sm"
            />
          </div>
        </div>
      </div>
